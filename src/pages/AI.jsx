      ...
      try {
        ...
        if (Array.isArray(importedMessages)) {
          const newChat = {
            ...
            title: importedMessages[0]?.content?.substring(0, 30) + "...",
          ...
          localStorage.setItem("aiChatHistory", JSON.stringify([newChat, ...chatHistory]));
        } else {
          alert("Файл не содержит корректную историю чата.");
        }
      } catch (err) {
          ...
          <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: 2, borderBottom: `1px solid rgba(0, 0, 0, 0.05)` }}>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center' }}>
              ...
              <Menu anchorEl={anchorEl} open={Boolean(anchorEl)} onClose={() => setAnchorEl(null)}>
                ...
                <MenuItem onClick={() => { document.getElementById('import-chat-input').click(); setAnchorEl(null); }}>
                  <Download /> {detectLanguage(navigator.language) === 'ru' ? 'Импорт' : 'Import'}
                </MenuItem>
